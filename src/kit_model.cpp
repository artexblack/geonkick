                   ...
                   "A5", "A#5", "B5", "C6", "Any"}
{
        ...
        RK_ACT_BIND(dspProxy, percussionUpdated, RK_ACT_ARGS(size_t id), this, updatePercussion(getIndex(id)));
}
...
{
        if (isValidIndex(index) && dspProxy->enablePercussion(percussionId(index))) {
...
{
        return dspProxy->isPercussionEnabled(percussionId(index));
}

void KitModel::selectPercussion(PercussionIndex index)
{
        if (isValidIndex(index) && dspProxy->setCurrentPercussion(percussionId(index))) {
...
{
        return static_cast<size_t>(percussionId(index)) == dspProxy->currentPercussion();
}
...
{
        return percussionsList[getIndex(dspProxy->currentPercussion())];
}
...
{
        return dspProxy->getPercussionChannel(percussionId(index));
}

bool KitModel::setPercussionChannel(PercussionIndex index, int channel)
{
        return dspProxy->setPercussionChannel(percussionId(index), channel);
}
...
{
        return dspProxy->getPercussionMidiChannel(percussionId(index));
}

bool KitModel::setPercussionMidiChannel(PercussionIndex index, int channel)
{
        return dspProxy->setPercussionMidiChannel(percussionId(index), channel);
}

bool KitModel::setPercussionName(PercussionIndex index, const std::string &name)
{
        if (dspProxy->setPercussionName(percussionId(index), name)) {
                dspProxy->notifyUpdateGui();
                return true;
        }
        return false;
...
{
        return dspProxy->getPercussionName(percussionId(index));
}
...
bool KitModel::setPercussionKey(PercussionIndex index, KeyIndex keyIndex)
{
        if (!isValidIndex(index))
                return false;
        if (dspProxy->setPercussionPlayingKey(percussionId(index), keyIndex)) {
                action percussionUpdated(percussionsList[index]);
                return true;
        }
        return false;
...
{
        return dspProxy->getPercussionPlayingKey(percussionId(index));
}
...
{
        dspProxy->playKick(percussionId(index));
}
...
bool KitModel::setPercussionLimiter(PercussionIndex index, int value)
{
        ...
        if (dspProxy->setPercussionLimiter(percussionId(index), realVal)) {
...
{
        auto realVal = dspProxy->percussionLimiter(percussionId(index));
...
{
        auto realVal = dspProxy->getLimiterLevelerValue(percussionId(index));
...
{
        return dspProxy->mutePercussion(percussionId(index), b);
}

bool KitModel::isPercussionMuted(PercussionIndex index) const
{
        return dspProxy->isPercussionMuted(percussionId(index));
}

bool KitModel::soloPercussion(PercussionIndex index, bool b)
{
        return dspProxy->soloPercussion(percussionId(index), b);
}

bool KitModel::isPercussionSolo(PercussionIndex index) const
{
        return dspProxy->isPercussionSolo(percussionId(index));
}
...
void KitModel::copyPercussion(PercussionIndex index)
{
        ...
        auto state = dspProxy->getPercussionState(percussionId(index));
        if (state) {
                state->setId(newId);
                state->enable(true);
                dspProxy->setPercussionState(state);
...
void KitModel::removePercussion(PercussionIndex index)
{
        ...
        for (auto it = percussionsList.begin(); it != percussionsList.end(); ++it) {
                if ((*it)->index() == index && dspProxy->enablePercussion(percussionId(index), false)) {
                        action percussionRemoved(index);
                        bool notify = (*it)->isSelected();
                        delete *it;
                        percussionsList.erase(it);
                        dspProxy->removeOrderedPercussionId(percussionId(index));
                        if (notify) {
                                dspProxy->setCurrentPercussion(percussionId(0));
...
{
        return dspProxy->getInstrumentBuffer(percussionId(index));
}

bool KitModel::enableNoteOff(PercussionIndex index, bool b)
{
        return dspProxy->enableNoteOff(percussionId(index), b);
}

bool KitModel::isNoteOffEnabled(PercussionIndex index) const
{
        return dspProxy->isNoteOffEnabled(percussionId(index));
}
